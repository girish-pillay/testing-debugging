...
async function openI2RTabAndPrintLastUpdated(page, tabName) {
  ...
  console.log(`📊 JALNA I2R → ${tabName} → ${text}`);
}

/* ================= MASD HELPER ================= */

async function extractMASDLastUpdated(page) {
  console.log('MASD for Jalna');
...
test('Jalna Complete Validation | UPC + MASD + I2R', async ({ page }) => {
  ...
  await loginPage.jalna();

  console.log('JALNA selected');
