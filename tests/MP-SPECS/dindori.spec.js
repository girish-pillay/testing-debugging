...
test('Org backup for Dindori districts', async ({ page }) => {
...
test('MASD for Dindori', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.goTo();
  await loginPage.ValidLogin(dataset.username, dataset.password);
  ...
  const masdTabs = ['Case Summary', 'Case Activities'];
...
test('User monitoring for Dindori', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.goTo();
  await loginPage.ValidLogin(dataset.username, dataset.password);
  //await page.getByRole('list').locator('a').nth(1).click();
...
test('Print Member Dashboard name and last updated date for Dindori', async ({ page }) => {
  const loginPage = new LoginPage(page);
  await loginPage.goTo();
  await loginPage.ValidLogin(dataset.username, dataset.password);
